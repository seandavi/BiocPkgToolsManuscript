...
\author[Davis \textit{et~al}.]{Sean Davis\,$^{\text{\sfb 1,}*}$, Lori Shepherd\,$^{\text{\sfb 2}}$, Martin T. Morgan\,$^{\text{\sfb 2}}$, and Vincent J. Carey\,$^{\text{\sfb 3,}}$}
...
\abstract{\textbf{Motivation:} The Bioconductor project, a large
  ...
  incorporated into the Bioconductor project. It provides easy access
  to tidy data structures of detailed package metadata, longitudinal
  package download statistics, package dependencies, and build
  reports. Conversion of package dependencies from tidy data
  structures to graph data structures is also possible, enabling
  graph-based analytics and visualization. \\ \textbf{Availability:}
  The BiocPkgTools software and complete documentation are available
  from Bioconductor
