...
\begin{abstract}
  \\ \textbf{Motivation:} The \emph{Bioconductor} project, a large collection of
  ...
  package dependencies, and \emph{Bioconductor} build reports, all in ``tidy
  ...
  software and complete documentation are available from \emph{Bioconductor}
...
the \emph{Bioconductor} project build successfully and pass basic
...
\begin{table}[h]
  ...
  \begin{tabularx}{\linewidth}{ l X }
    ...
    biocExplore & Interactively browsable ``bubble plot'' of \emph{Bioconductor} packages and details \\
    ...
    inducedSubgraphByPkgs & Create a minimal subgraph of \emph{Bioconductor} dependencies bases on specific packages \\
...
As an alternative to google and the \emph{Bioconductor} online software list, the \texttt{biocExplore} function offers interactive and graphical approach to package browsing (see Figure \ref{fig:biocexplore}). 
